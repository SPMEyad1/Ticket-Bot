...
client.on(`message`, async (message) => {
  ...
  if (message.content.startsWith(`${prefix}embed`)) {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setFooter("TroubleMaker Music", client.user.displayAvatarURL())
    //delete the Command
    ...
    if (now < expirationTime) {
      ...
      return message.reply(
        new MessageEmbed().setColor("#ff0000")
          .setTitle(`<:X_:898650740564840499> Please wait \`${timeLeft.toFixed(1)} seconds\` before reusing the \`${prefix}${command.name}\`!`)
      );
    }
  }
  ...
  try {
    ...
    message.reply(new MessageEmbed().setColor("#ff0000")
      .setTitle(`<:X_:898650740564840499>There was an error executing that command.`)).catch(console.error);
  }
